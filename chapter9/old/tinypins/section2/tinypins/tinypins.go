...
func handlePinsGetUpdate(w http.ResponseWriter, r *http.Request) {
	...
	cacheItem, err := memcache.Get(ctx, id)
	if err == nil && cacheItem != nil { // return directly from cache
		log.Infof(ctx, "memcache get: %s", string(cacheItem.Value))
		_, err := w.Write(cacheItem.Value)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	if err != nil && err != memcache.ErrCacheMiss {
		log.Errorf(ctx, "memcache get: %s", err)
	}
	...
	err = json.NewEncoder(&buf).Encode(pins)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	log.Infof(ctx, "memcache set: %s", buf.String())
	err = memcache.Set(ctx, cacheItem)
	if err != nil {
		log.Errorf(ctx, "memcache set: %s", err)
	}
}
...
func handlePinsPostUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil && err != memcache.ErrCacheMiss {
		log.Errorf(ctx, "memcache delete: %s", err)
	}
